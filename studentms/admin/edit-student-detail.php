...
if (strlen($_SESSION['sturecmsaid']==0)) {
   ...
   if(isset($_POST['submit']))
  {
                ...
                $cname="";
                $sql="SELECT * from tblbatch  where BatchName=:stubatch";
                $query = $dbh -> prepare($sql);
                $query->bindParam(':stubatch',$stubatch,PDO::PARAM_STR);
                $query->execute();
              ...
              $sql="update tblstudent set StudentName=:stuname,StudentEmail=:stuemail,StudentBatch=:stubatch,CourseName=:cname,Semester=:stusem,Gender=:gender,DOB=:dob,StuID=:stuid,FatherName=:fname,MotherName=:mname,ContactNumber=:connum,AltenateNumber=:altconnum,Address=:address where ID=:eid";
              $query=$dbh->prepare($sql);
              $query->bindParam(':stuname',$stuname,PDO::PARAM_STR);
              $query->bindParam(':stuemail',$stuemail,PDO::PARAM_STR);
              $query->bindParam(':stubatch',$stubatch,PDO::PARAM_STR);
              $query->bindParam(':cname',$cname,PDO::PARAM_STR);
              $query->bindParam(':stusem',$stusem,PDO::PARAM_STR);
...
{               ?>
                      ...
                      <div class="form-group">
                        <label for="exampleInputEmail3">Student Batch</label>
                        <select  name="stubatch" class="form-control" required='true'>
                        <option value="<?php  echo htmlentities($row->StudentBatch);?>"><?php  echo htmlentities($row->StudentBatch);?></option>
...
<option value="<?php echo htmlentities($row1->BatchName);?>"><?php echo htmlentities($row1->BatchName);?></option>
 <?php } ?> 
                      ...
                      <div class="form-group">
                        <label for="exampleInputName1">Current Semester</label>
                        <select name="stusem" value="" class="form-control" required='true'>
